...
<?php
	if(isset($_POST['Finish']))
	{
		...
		$que_user_data=mysql_query("select * from users where user_name='$user';");
		$user_data=mysql_fetch_array($que_user_data);
		$userid=$user_data[0];
		$user_join_time=$user_data[6];
		mysql_query("insert into user_post(user_id,post_txt,post_time,priority) values($userid,'Joined Premierian :).','$user_join_time','Public');");
...
<select name="que" style="height:38;font-size:18px;padding:3;">
		...
		<option value="what was the make of your fist car?">what was the make of your fist mobile phone?</option>
